...
const initial = (): ProviderForm => ({
  ...
  models: [{ alias: 'default', model: '', inputPrice: '0', outputPrice: '0' }], apiKeyEnv: 'env:MCP_RIKSA_PROVIDER_API_KEY',
  headerEnv: '',
});
...
export function SettingsPage({ providers, workspace, onRefresh }: { providers: ProviderSummary[]; workspace?: WorkspaceSummary; onRefresh(): Promise<void> }) {
  ...
  const changeModel = (index: number, key: 'alias' | 'model' | 'inputPrice' | 'outputPrice', value: string) => setForm((current) => ({
    ...
    <Section title={workspace ? 'Repository configuration' : editingId ? `Edit provider · ${editingId}` : 'Add model provider'} action={workspace ? <Status value="read only" /> : editingId ? <Status value="editing" /> : undefined}>
      {workspace ? <Notice>Providers come from <code>{workspace.configPath}</code>. Edit that file and restart MCP Riksa to apply changes. Provider connection tests remain available here.</Notice> : <form className="form-grid" onSubmit={(event) => { event.preventDefault(); void act(async () => {
        ...
        <div className="model-editor">
          <header><div><span className="eyebrow">Model catalog</span><b>{form.models.length} configured</b></div><Button type="button" onClick={() => change('models', [...form.models, { alias: '', model: '', inputPrice: '0', outputPrice: '0' }])}>＋ Add model</Button></header>
          <div className="model-rows">{form.models.map((entry, index) => <div className="model-row" key={index}>
            ...
            <Field label="Input $ / 1M"><Input required inputMode="decimal" value={entry.inputPrice} onChange={(event) => changeModel(index, 'inputPrice', event.target.value)} /></Field>
            <Field label="Output $ / 1M"><Input required inputMode="decimal" value={entry.outputPrice} onChange={(event) => changeModel(index, 'outputPrice', event.target.value)} /></Field>
        ...
        <Field label="API key reference" hint="env:ENV_NAME, vault:secret-id, or session:secret-id."><Input value={form.apiKeyEnv} onChange={(event) => change('apiKeyEnv', event.target.value)} /></Field>
