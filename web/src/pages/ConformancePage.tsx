...
export function ConformancePage({ reports, servers, initialId, onRefresh }: {
  ...
  return <div className="page-grid conformance-page">
    <aside className="workspace-stack conformance-sidebar">
      <Section className="conformance-runner" title="Official runner" action={<Status value="HTTP only" />}>
        <p className="section-copy conformance-runner-copy">
          Pinned runner <code>@modelcontextprotocol/conformance@0.1.10</code>. Loopback Streamable HTTP only. Stdio and authenticated endpoints are unsupported.
        </p>
        <div className="form-grid conformance-runner-form">
        ...
        <div className="button-row conformance-runner-actions"><Button variant="primary" disabled={!serverId || (mode === 'scenario' && !scenario.trim())} onClick={() => void start()}>Run official checks</Button></div>
        {error ? <Notice error>{error}</Notice> : null}
        <p className="hint conformance-disclaimer">Passing applies only to the selected scenarios—it is not universal MCP certification.</p>
      </Section>
      <Section className="conformance-history" title="History" action={<span className="count">{reports.length}</span>}>
    ...
    <main className="workspace-stack conformance-results">
      {!selected ? <Section className="conformance-report" title="Report"><Empty>Select a historical report or start a check run.</Empty></Section> : <>
        <Section className="conformance-report" title="Report" action={<Status value={selected.status} />}>
          <div className="conformance-metrics">
            <div className="pass"><b>{selected.summary.passed}</b><span>Passed</span></div>
            <div className="fail"><b>{selected.summary.failed}</b><span>Failed</span></div>
            <div className="warning"><b>{selected.summary.warnings}</b><span>Warnings</span></div>
            <div><b>{selected.summary.skipped}</b><span>Skipped</span></div>
            <div className="fail"><b>{selected.summary.harnessErrors}</b><span>Harness errors</span></div>
          </div>
          <div className="conformance-summary">
            <p>{selected.status === 'passed' ? 'All tested scenarios passed.' : 'Review scenario checks and harness diagnostics below.'}</p>
            <p className="hint">Runner {selected.runnerVersion} · <span className="conformance-endpoint">{selected.endpoint}</span></p>
          </div>
