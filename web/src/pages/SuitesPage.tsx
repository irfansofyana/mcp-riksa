...
function UniqueIdInput({ value, editorId, testId, duplicateMessage, onChange, onError }: {
  value: string;
  editorId: string;
  testId?: string;
  duplicateMessage(value: string): string | undefined;
  ...
  return <Input value={value} data-testid={testId} onChange={(event) => {
    const next = event.target.value;
    const issue = duplicateMessage(next);
    if (issue) { onError(editorId, issue); return; }
    onChange(next);
    onError(editorId, '');
  }} />;
}
...
function AssertionEditor({ assertion, index, tools, types, onChange, onRemove, onError, editorId, scope }: {
  ...
  onErrorRef.current = onError;
  useEffect(() => () => {
    for (const key of pendingKeys.current) onErrorRef.current(key, '');
  }, []);
  const reportError = (field: string, message: string) => {
    const key = `${editorId}:${field}`;
    if (message) pendingKeys.current.add(key); else pendingKeys.current.delete(key);
    onError(key, message);
  };
  ...
  const jsonBlur = (field: string, value: string, change: (value: JsonValue) => void) => {
    try { change(parseJson(value)); reportError(field, ''); } catch { reportError(field, 'Expected value must be valid JSON.'); }
  };
...
export function SuitesPage({ suites, servers, providers, onRefresh, onRunStarted }: {
  ...
  const [pendingEditorIssues, setPendingEditorIssues] = useState<Record<string, string>>({});
  const [hasIntentionalDraft, setHasIntentionalDraft] = useState(false);
  const [creationOpen, setCreationOpen] = useState(false);
  const assertionEditorIds = useRef(createEditorIdentityRegistry<SuiteAssertion>('assertion')).current;
  ...
  const pendingEditorMessage = Object.values(pendingEditorIssues)[0];
  const hasPendingEditorChanges = pendingEditorMessage !== undefined;
  ...
  const handleEditorError = (editorId: string, message: string) => {
    setPendingEditorIssues((current) => updatePendingEditorIssues(current, editorId, message));
  ...
  const renderCaseEditor = () => {
    ...
    return <div className="case-composer">
      ...
      <div className="case-section"><span className="eyebrow">Scenario</span><div className="form-grid">
        <Field label="Case ID" hint="Stable unique ID used in reports"><UniqueIdInput value={activeCase.id} editorId={`case:${activeCase.id}:id`} testId="case-id" duplicateMessage={(nextId) => draft.cases.some((entry) => entry.id !== activeCase.id && entry.id === nextId) ? `Case ID ${nextId} already exists.` : undefined} onError={handleEditorError} onChange={(nextId) => { const previous = activeCase.id; const next = { ...activeCase, id: nextId } as SuiteCase; applyDraft({ ...draft, cases: draft.cases.map((entry) => entry.id === previous ? next : entry) }); setActiveCaseId(nextId); }} /></Field>
      ...
      </div>{isV2AgentCase(activeCase) ? <div className="case-section assertions-editor"><header><div><span className="eyebrow">Scripted conversation</span><h3>{activeCase.turns.length} user turns</h3></div><Button onClick={() => updateV2Turns([...activeCase.turns, { id: nextTurnId(activeCase.turns), user: '', assertions: [] }])}>+ User turn</Button></header>{activeCase.turns.map((turn, turnIndex) => <div className="assertion-card" data-testid={`turn-editor-${turnIndex}`} key={`${activeCase.id}-${turn.id}`}><header><span><b>{turnIndex + 1}</b> User turn</span><div className="config-actions compact"><Button onClick={() => { if (turnIndex === 0) return; const turns = [...activeCase.turns]; [turns[turnIndex - 1], turns[turnIndex]] = [turns[turnIndex]!, turns[turnIndex - 1]!]; updateV2Turns(turns); }}>↑</Button><Button onClick={() => { if (turnIndex === activeCase.turns.length - 1) return; const turns = [...activeCase.turns]; [turns[turnIndex], turns[turnIndex + 1]] = [turns[turnIndex + 1]!, turns[turnIndex]!]; updateV2Turns(turns); }}>↓</Button><Button variant="danger" data-testid={`remove-turn-${turnIndex}`} disabled={activeCase.turns.length === 1} onClick={() => updateV2Turns(activeCase.turns.filter((_turn, index) => index !== turnIndex))}>Remove</Button></div></header><div className="assertion-fields"><Field label="Turn ID"><UniqueIdInput value={turn.id} editorId={`case:${activeCase.id}:turn:${turnIndex}:id`} testId={`turn-id-${turnIndex}`} duplicateMessage={(id) => activeCase.turns.some((entry, index) => index !== turnIndex && entry.id === id) ? `Turn ID ${id} already exists.` : undefined} onError={handleEditorError} onChange={(id) => updateV2Turns(activeCase.turns.map((entry, index) => index === turnIndex ? { ...entry, id } : entry))} /></Field><Field label="User message"><Textarea rows={4} value={turn.user} onChange={(event) => updateV2Turns(activeCase.turns.map((entry, index) => index === turnIndex ? { ...entry, user: event.target.value } : entry))} /></Field></div><div className="case-section assertions-editor"><header><div><span className="eyebrow">Turn assertions</span><h3>{turn.assertions.length} checks</h3></div><Select aria-label={`Add assertion to ${turn.id}`} value="" onChange={(event) => { if (!event.target.value) return; updateV2Turns(activeCase.turns.map((entry, index) => index === turnIndex ? { ...entry, assertions: [...entry.assertions, createSuiteAssertion(event.target.value as SuiteAssertion['type'])] } : entry)); event.target.value = ''; }}><option value="">+ Add check</option>{assertionTypes.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}</Select></header>{turn.assertions.length === 0 ? <Empty>No turn checks yet.</Empty> : turn.assertions.map((assertion, assertionIndex) => <AssertionEditor key={`${assertionEditorIds.id(assertion)}-${assertion.type}`} assertion={assertion} index={assertionIndex} editorId={assertionEditorIds.id(assertion)} scope={turn.id} tools={tools} types={assertionTypes} onError={handleEditorError} onChange={(value) => updateV2Turns(activeCase.turns.map((entry, index) => index === turnIndex ? { ...entry, assertions: entry.assertions.map((current, currentIndex) => currentIndex === assertionIndex ? assertionEditorIds.transfer(assertion, value) : current) } : entry))} onRemove={() => updateV2Turns(activeCase.turns.map((entry, index) => index === turnIndex ? { ...entry, assertions: entry.assertions.filter((_current, currentIndex) => currentIndex !== assertionIndex) } : entry))} />)}</div></div>)}</div> : null}<div className="form-grid compact suite-limits">
      ...
      <div className="case-section assertions-editor"><header><div><span className="eyebrow">Expected behavior</span><h3>{activeCase.assertions.length} checks</h3></div><Select aria-label="Add assertion" value="" onChange={(event) => { if (!event.target.value) return; updateAssertions([...activeCase.assertions, createSuiteAssertion(event.target.value as SuiteAssertion['type'])]); event.target.value = ''; }}><option value="">+ Add check</option>{availableAssertionTypes.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}</Select></header>
        {activeCase.assertions.length === 0 ? <Empty>No checks yet. Add expected tools, output checks, or execution budgets.</Empty> : activeCase.assertions.map((assertion, index) => <AssertionEditor key={`${assertionEditorIds.id(assertion)}-${assertion.type}`} assertion={assertion} index={index} editorId={assertionEditorIds.id(assertion)} tools={tools} types={availableAssertionTypes} onError={handleEditorError} onChange={(value) => updateAssertions(activeCase.assertions.map((entry, assertionIndex) => assertionIndex === index ? assertionEditorIds.transfer(assertion, value) : entry))} onRemove={() => updateAssertions(activeCase.assertions.filter((_entry, assertionIndex) => assertionIndex !== index))} />)}
      </div>
    </div>;
  };
  ...
  return <><SuiteCreateLaunchpad open={creationOpen} servers={servers} providers={providers} suiteNames={suites} onClose={() => setCreationOpen(false)} onCreate={acceptCreatedDocument} /><div className="suite-studio">
    ...
    <Section title="Suite composer" className="suite-workspace" action={<div className="suite-view-tabs"><button disabled={busy} className={view === 'builder' ? 'selected' : ''} onClick={() => switchView('builder')}>Builder</button><button disabled={busy} className={view === 'yaml' ? 'selected' : ''} onClick={() => switchView('yaml')}>YAML</button></div>}>
      ...
      {loading ? <div className="loading"><i />Loading suite…</div> : <>
        ...
        {message ? <Notice>{message}</Notice> : null}{error || pendingEditorMessage || document.sourceError || document.loadError ? <Notice error>{error || pendingEditorMessage || document.sourceError || document.loadError}</Notice> : null}
      </>}
      </fieldset>
    </Section>
  </div></>;
