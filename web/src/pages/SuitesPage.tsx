...
function AssertionEditor({ assertion, index, tools, onChange, onRemove, onError }: {
  ...
  const toolInput = (value: string, change: (value: string) => void) => <ToolInput id={`assertion-tool-${index}`} value={value} tools={tools} onChange={change} />;
  ...
  return <div className="assertion-card">
    ...
    <div className="assertion-fields">
      <Field label="Assertion"><Select value={assertion.type} onChange={(event) => onChange(createSuiteAssertion(event.target.value as SuiteAssertion['type']))}>{assertionTypes.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}</Select></Field>
  ...
  const renderCaseEditor = () => {
    ...
    return <div className="case-composer">
      ...
      {activeCase.kind === 'direct' ? <div className="case-section"><span className="eyebrow">Direct invocation</span><div className="form-grid">
        ...
        <Field label="User prompt" hint="One scenario turn; save playground interactions for richer seeds"><Textarea rows={7} value={activeCase.prompt} onChange={(event) => replaceCase({ ...activeCase, prompt: event.target.value })} /></Field>
      </div><div className="form-grid compact suite-limits">
        <Field label="Max turns"><Input type="number" min="1" max="50" value={activeCase.limits.maxTurns} onChange={(event) => replaceCase({ ...activeCase, limits: { ...activeCase.limits, maxTurns: Number(event.target.value) } })} /></Field>
      ...
      <div className="case-section assertions-editor"><header><div><span className="eyebrow">Expected behavior</span><h3>{activeCase.assertions.length} checks</h3></div><Select aria-label="Add assertion" value="" onChange={(event) => { if (!event.target.value) return; updateAssertions([...activeCase.assertions, createSuiteAssertion(event.target.value as SuiteAssertion['type'])]); event.target.value = ''; }}><option value="">+ Add check</option>{assertionTypes.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}</Select></header>
        {activeCase.assertions.length === 0 ? <Empty>No checks yet. Add expected tools, output checks, or execution budgets.</Empty> : activeCase.assertions.map((assertion, index) => <AssertionEditor key={`${draft.name}-${activeCase.id}-${index}-${assertion.type}`} assertion={assertion} index={index} tools={tools} onError={setError} onChange={(value) => updateAssertions(activeCase.assertions.map((entry, assertionIndex) => assertionIndex === index ? value : entry))} onRemove={() => updateAssertions(activeCase.assertions.filter((_entry, assertionIndex) => assertionIndex !== index))} />)}
      </div>
    </div>;
  };
  ...
  return <div className="suite-studio">
    ...
    <Section title="Suite composer" className="suite-workspace" action={<div className="suite-view-tabs"><button className={view === 'builder' ? 'selected' : ''} onClick={() => switchView('builder')}>Builder</button><button className={view === 'yaml' ? 'selected' : ''} onClick={() => switchView('yaml')}>YAML</button></div>}>
      {loading ? <div className="loading"><i />Loading suite…</div> : <>
        <div className="suite-meta"><Field label="Suite name" hint="Filename-safe ID used by CLI and reports"><Input value={draft.name} onChange={(event) => applyDraft({ ...draft, name: event.target.value })} data-testid="suite-name" /></Field><Field label="Description" hint="Optional intent for reviewers"><Input value={draft.description ?? ''} onChange={(event) => applyDraft({ ...draft, description: event.target.value })} /></Field></div>
        {view === 'yaml' ? <div className="yaml-workspace"><div className="yaml-note"><span>Canonical artifact</span><b>Version 1 YAML</b><small>Edit directly, commit it, or run with <code>mcp-workbench run</code>.</small></div><Field label="Suite source"><Textarea className="code-editor" rows={28} value={source} onChange={(event) => setSource(event.target.value)} spellCheck={false} data-testid="suite-source" /></Field></div> : <div className="suite-builder">
